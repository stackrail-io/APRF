...
Local by default; drop runtime exports under `aprf-assessment/imports/<plugin>/`; live APIs only with `APRF_AUDITOR_LIVE=1`. Hybrid secrets / supply-chain / tool-safety collectors need measured imports — see `capabilities.yaml` and `workflow.md`.
