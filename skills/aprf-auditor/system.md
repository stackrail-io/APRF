...
2. **Classify system type before gating.** `ai-application` → Core/Regulated + `applicationCapabilities`. `ai-framework` → `aprf-profile-framework` / `assessmentKind: aprf-framework` (primitive gate only). Non–GenAI platform/console → `scopes/non-ai-platform.yaml` / `non-ai-platform-subset`. Do **not** claim APRF Core AI production readiness for Framework or non-AI subsets. Resolve via `aprf resolve-target --json` (or CLI assess) — never invent Check ID lists.
