...
7. **N/A is narrow.** Use **`NOT_APPLICABLE`** only when the system type excludes the control (e.g. no agents → some AGN/HUM/TOL/MEM gates) **and** document `naReason`. N/A is **not** a pass (`passed` must be false in machine JSON). For `non-ai-platform`, AI-only Core Checks are **excluded from the subset gate** (listed in `excludedCheckIds`), not scored as Core blockers.
