...
- Rewrote **authorization** **AUTHZ-M3** (hybrid agent/automation role matrix with non-admin defaults + ≤90d access review / 0 unexplained escalations; `agent-role-matrix` collector + measuredAt ≤90d; N/A via `productionAgentOrAutomationIdentitiesPresent=false`); cleared `technologies`; synced `aprf-spec.json`.
- Rewrote **authorization** **AUTHZ-M2** (hybrid multi-tenant AI data/memory cross-tenant attack suite ≥10 cases / 0 leaks; `cross-tenant-tests` collector + measuredAt ≤90d; N/A via `multiTenantAiDataOrMemoryPathsPresent=false`); cleared `technologies`; synced `aprf-spec.json`.
- Rewrote **authorization** **AUTHZ-M1** (hybrid server-side authz on privileged AI feature/tool/retrieval entry points + authenticated-but-unauthorized suite; `authz-entry-tests` collector + measuredAt ≤90d; N/A via `privilegedAiFeatureToolOrRetrievalEntryPointsPresent=false`); cleared `technologies`; synced `aprf-spec.json`.
