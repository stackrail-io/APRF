...
- N/A existence gates in `passCondition` for reliability, model-lifecycle, and agents Checks (synced into `aprf-spec.json`); polished gate wording (explicit subjects; PRM-M3/EVL/AGN-R1/PRM-R2 scope fixes); **CHG-M1** evidence scoped to in-use prompt and/or model-pin artifacts.
...
Published on npm as `@stackrail-io/aprf-engine@0.11.0` and `@stackrail-io/aprf-framework-definition@0.11.0`. Spec `governance.version` → **0.11.0**.
