...
### Changed
- Rewrote **secrets** **SEC2-M1** (hybrid secrets-manager + prompt/fixture secret scan; `repo-secrets-hygiene` / `secrets-hygiene` collector requires measuredAt ≤90d coverage—no vacuous PASS from CI scan config alone; N/A via `productionRuntimeSecretsPresent=false`; cleared `technologies`); synced `aprf-spec.json`.
- Rewrote **secrets** **SEC2-M2** (hybrid log/trace redaction + canary; `repo-secret-redaction` / `secret-redaction` requires detectionRatePct=100 + pattern coverage + measuredAt ≤90d; N/A via `productionLoggingOrTracingPipelinesPresent=false`; cleared `technologies`); synced `aprf-spec.json`.
- Rewrote **secrets** **SEC2-M3** (hybrid key inventory + least-privilege scope + rotation/short-lived + 0 privileged client keys; `repo-key-rotation-scope` / `key-rotation-scope` requires measuredAt ≤90d coverage—no vacuous PASS from rotation docs alone; N/A via `productionProviderOrCloudKeysPresent=false`; cleared `technologies`); synced `aprf-spec.json`.
- Rewrote **secrets** **SEC2-R1** (hybrid pre-commit + CI secret scanning covering prompts/fixtures with blocking + ≤7d green main/PR-merge scan; `repo-precommit-ci-secret-scan` / `precommit-ci-secret-scan`—no vacuous PASS from config alone; replaced unused `repo-secret-scan-config`/`repo-pre-commit-config` detectors; N/A via `applicationCodePromptsOrFixturesPresent=false`; cleared `technologies`); synced `aprf-spec.json`.
- Rewrote **secrets** **SEC2-R2** (hybrid credential egress allowlist + documented destinations + ≥1 deny event ≤90d; `repo-credential-egress-controls` / `credential-egress-controls`—no vacuous PASS from allowlist docs alone; distinct from SEC-M4/INF-M3; N/A via `runtimesHoldingCredentialsPresent=false`; cleared `technologies`); synced `aprf-spec.json`.
- Rewrote **secrets** **SEC2-R3** (hybrid dataset secret/PII scan gate before fine-tune/eval publish + 100% linked scan reports ≤90d + blocking on critical findings; `repo-dataset-secret-scan-gate` / `dataset-secret-scan-gate`—no vacuous PASS from dataset cards alone; distinct from SEC2-R1/DG-M2/DG-R3; N/A via `fineTuneOrEvalCorpusPublishPresent=false`; cleared `technologies`); synced `aprf-spec.json`.
