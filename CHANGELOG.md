...
### Changed
- Catalog P1 tooling: `build-catalog` runs full YAML lint/path checks; CI diffs `evidence-type-ids.ts` with `catalog.ts`; pruned unused detector allowlist entries; INF siblings `relatedRules` point at `SCI-R1` instead of deprecated `INF-R1`.
