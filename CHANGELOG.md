...
### Changed
- Rewrote **tool-safety** **TOL-M1**–**TOL-M5** and **TOL-R1**–**TOL-R2** (hybrid; measuredAt ≤90d; N/A when out of scope; cleared `technologies`; synced `aprf-spec.json`) and collectors (`tool-gateway-authz`, `tool-allowlist`, `high-impact-tool-gates`, `tool-argument-schema`, `signed-tool-catalog`, `destructive-tool-dry-run`, `tool-rate-limits`): **M1** server-side tool-gateway authz independent of model output; **M2** per-agent allowlist + unknown-tool deny; **M3** high-impact extra gates; **M4** argument schema + contract reject; **M5** Level-5 signed MCP/agent tool catalogs; **R1** destructive dry-run in non-prod; **R2** per-tool rate + blast-radius budgets with ≤30d enforcement proof.
