...
| Created | 2026-08-10 |
| SemVer impact | MINOR |
| Index summary | Adds informative section-level crosswalks for AISVS, ASVS, OpenCRE, MAESTRO, and FIASSE; enriches OWASP LLM Top 10 controls with AISVS `relatedPeerControlIds` bridges. |
...
The LLM Top 10 crosswalk exists, but it does not surface the AISVS section bridges that peer playbooks publish alongside each LLM risk.

## Proposal

1. Add five informative frameworks under `spec/aprf-spec.json` → `crosswalks[]`:
   - **aisvs** — 89 AISVS sections (`C*.*`)
...
2. Extend peer control shape with optional `relatedPeerControlIds` and populate OWASP LLM Top 10 controls with AISVS section IDs (e.g. LLM01 → `aisvs:C2.1`, …).
...
Crosswalks remain **informative alignment only** — not certification or proof of compliance. Peer markdown corpora are **not** vendored into APRF.

## Alternatives considered

- Vendor playbook `data/*` corpora — rejected for this RFC; APRF keeps ID/title/summary maps only.
